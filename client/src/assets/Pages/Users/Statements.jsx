...
const TABS = ["contributions", "savings", "loans", "repayments"];
...
const Statements = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [contributionsRes, savingsRes, loansRes] = await Promise.all([
      ...
      try {
        const repaymentsRes = await axios.get("/loan-payments/my");
      ...
      setSummary({
        contributions: contributionsRes.data.reduce(
          (sum, item) => sum + Number(item.amount || 0),
          0,
        ),

        savings: savingsRes.data.reduce(
          (sum, item) => sum + Number(item.amount || 0),
          0,
        ),

        loans: loansRes.data.reduce(
          (sum, item) => sum + Number(item.amount || 0),
          0,
        ),
      ...
      Swal.fire("Error", "Failed to load statement", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(workbook, `my-${activeTab}.xlsx`);
  };
  ...
  return (
    <div className="space-y-4 p-4 text-[11px]">
      ...
      <div className="bg-white border rounded-xl p-3 flex items-center gap-2">
        ...
        <h2 className="font-semibold text-[11px]">My Statements</h2>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        <div className="bg-white border rounded-xl p-3">
          ...
          <p className="font-semibold">
            KES {formatCurrency(summary.contributions)}
          </p>
        </div>
        ...
        <div className="bg-white border rounded-xl p-3">
          ...
          <p className="font-semibold">KES {formatCurrency(summary.savings)}</p>
        </div>

        <div className="bg-white border rounded-xl p-3">
          <p className="text-[10px] text-gray-500">Loans</p>

          <p className="font-semibold">KES {formatCurrency(summary.loans)}</p>
        </div>
        ...
        <div className="bg-white border rounded-xl p-3">
          ...
          <p className="font-semibold">
            KES {formatCurrency(summary.repayments)}
          </p>
        </div>
        ...
        <div className="bg-white border rounded-xl p-3">
          ...
          <p className="font-semibold text-red-600">
            KES {formatCurrency(summary.balance)}
          </p>
        </div>
      </div>
              ...
              <tr>
                {activeTab === "loans" ? (
                  <>
                    ...
                    <th className="px-3 py-2 text-left">Amount Borrowed</th>
                ...
                activeTab === "loans" ? (
                  paginatedData.map((loan) => (
                    <tr key={loan.id} className="border-b hover:bg-gray-50">
                      ...
                      <td className="px-3 py-2">{loan.username}</td>

                      <td className="px-3 py-2">{loan.group_name}</td>
                      ...
                      <td className="px-3 py-2 font-semibold text-red-600">
                        KES {Number(loan.balance || 0).toLocaleString()}
                      </td>
