...
import WalletAllocationModal from "./Modals/WalletAllocationModal";
...
const Wallet = () => {
  ...
  */
  ...
  const fetchWalletDeposits = async () => {
    try {
      ...
      const data =
        response.data?.deposits || response.data?.data || response.data || [];
      ...
      Swal.fire({
        ...
        title: "Error",
        text:
          error.response?.data?.message || "Failed to load wallet deposits.",
      });
    }
  };
  ...
  */
  ...
  const handleVerify = async (deposit) => {
    ...
    try {
      await axios.patch(`/wallet-deposits/${deposit.id}/verify`);
      ...
      Swal.fire({
        ...
        text:
          error.response?.data?.message || "Failed to verify wallet deposit.",
      });
    } finally {
  ...
  */
  ...
  const handleReject = async (deposit) => {
    ...
    try {
      await axios.patch(`/wallet-deposits/${deposit.id}/reject`, {
      ...
      Swal.fire({
        ...
        text:
          error.response?.data?.message || "Failed to reject wallet deposit.",
      });
    } finally {
      setProcessingId(null);
    }
  ...
  */
  ...
  const filteredDeposits = useMemo(() => {
    return deposits.filter((deposit) => {
      ...
      const matchesSearch =
        deposit.fullname?.toLowerCase().includes(searchValue) ||
        deposit.username?.toLowerCase().includes(searchValue) ||
        deposit.mpesa_code?.toLowerCase().includes(searchValue) ||
        deposit.bank_reference?.toLowerCase().includes(searchValue);

      const matchesStatus =
        statusFilter === "all" || deposit.status === statusFilter;

      const matchesPayment =
        paymentFilter === "all" || deposit.payment_method === paymentFilter;

      return matchesSearch && matchesStatus && matchesPayment;
    });
  }, [deposits, search, statusFilter, paymentFilter]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="bg-white border rounded-lg p-3">
          ...
          <p className="text-[9px] text-gray-400">All wallet transactions</p>
        </div>
        ...
        <div className="bg-white border rounded-lg p-3">
          ...
          <p className="text-[9px] text-gray-400">
            KES {stats.verifiedAmount.toLocaleString()}
          </p>
        </div>
        ...
        <div className="bg-white border rounded-lg p-3">
          <p className="text-[10px] text-gray-500">Available Wallet</p>

          <p className="text-lg font-semibold text-blue-600 mt-1">
            KES {stats.availableWallet.toLocaleString()}
          </p>

          <p className="text-[9px] text-gray-400">Available for allocation</p>
        </div>
      </div>
      ...
      <div className="bg-white border rounded-lg overflow-hidden">
                  ...
                  <tr key={deposit.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">
                      <div className="font-medium text-gray-800">
                        {deposit.fullname || "Unknown Member"}
                      </div>
                    ...
                    <td className="px-3 py-2 font-medium">
                      KES {Number(deposit.amount || 0).toLocaleString()}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex justify-end items-center gap-1">
                        ...
                        {deposit.status === "verified" &&
                          Number(deposit.remaining_balance || 0) > 0 && (
                            <button
                              onClick={() => {
                                setSelectedDeposit(deposit);
        ...
        <div className="px-3 py-2 bg-gray-50 text-[10px] text-gray-500">
          Showing {filteredDeposits.length} deposit(s)
        </div>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <p className="text-[9px] text-gray-400">Username</p>
                ...
                <div>
                  ...
                  <p className="text-[11px] font-semibold">
                    KES {Number(selectedDeposit.amount || 0).toLocaleString()}
                  </p>
                </div>

                <div>
                  <p className="text-[9px] text-gray-400">Remaining Balance</p>

                  <p className="text-[11px] font-semibold text-blue-600">
                    KES{" "}
                    {Number(
                      selectedDeposit.remaining_balance || 0,
                    ).toLocaleString()}
                  </p>
                </div>

                <div>
                  <p className="text-[9px] text-gray-400">Payment Method</p>
                ...
                <div>
                  <p className="text-[9px] text-gray-400">M-Pesa Code</p>
                ...
                <div>
                  <p className="text-[9px] text-gray-400">Bank Reference</p>
            ...
            <div className="border-t px-4 py-3 flex justify-end gap-2">
              ...
              {selectedDeposit.status === "verified" &&
                Number(selectedDeposit.remaining_balance || 0) > 0 && (
                  <button
                    onClick={() => {
      ...
      {showAllocation && selectedDeposit && (
